...
var esmUtils = require('./esm-utils');
...
Mocha.prototype.loadFiles = function(fn) {
  ...
  for (var i in this.files) {
    ...
    var result = esmUtils.requireOrImport(file);
    if (utils.isPromise(result)) return loadEsmOrCjs(this.files.slice(i));
    suite.emit(EVENT_FILE_REQUIRE, result, file, self);
    suite.emit(EVENT_FILE_POST_REQUIRE, global, file, self);
  }
  fn && fn();

  function loadEsmOrCjs(files) {
    return esmUtils
      .loadFilesAsync(
        files,
        function(file, i) {
 ...
 * @return {Runner|Promise<Runner>} runner instance
 */
Mocha.prototype.run = function(fn) {
  var loadResult = this.files.length ? this.loadFiles() : undefined;

  return utils.isPromise(loadResult)
    ? loadResult.then(next.bind(this))
    : next.bind(this)();
